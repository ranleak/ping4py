...
By default, Linux distributions ship ```ping``` as part of the larger ```iputils``` suite, which includes tools like ```arping``` and ```clockdiff```. For this project, you'll be compiling *only* the ```ping``` utility into a standalone binary that can be bundled into Python wheels or custom environments.
